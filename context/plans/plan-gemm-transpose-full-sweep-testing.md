...
3) Produces a single output directory under `tmp/` containing raw NVBench JSON outputs plus a normalized `results.json` (schema-valid).
4) Generates program reports **as a separate stage** (so reporting can be re-run without re-running NVBench):
   - `report.md` showing **only measured times** (plus selected `algo_id`) for quick scanning / sanity checks.
   - `all_timings.md` as a single “giant table” listing **all measured timings** and the **chosen cuBLASLt algorithm** for each record.
   - `stakeholder_report.md` template (human-in-the-loop; not auto-filled end-to-end).
...
4. Add a reporting entry point that consumes `results.json` and writes `report.md` and `all_timings.md` without running NVBench.
5. Add a resumable mechanism so partial runs can be continued (skip already-present records).
...
- Expected artifacts:
  ...
  - `report.md` (generated by `gemm-transpose report`; measured-time-only tables)
  - `all_timings.md` (generated by `gemm-transpose report`; giant table listing all records + measured time + selected algo)
  - `stakeholder_report.md` (created by `gemm-transpose report` if missing; human-in-the-loop narrative + analysis)
...
sequenceDiagram
  ...
  Dev->>Pixi: pixi run -e cuda13<br/>gemm-transpose report
  Pixi->>Or: render reports from results.json
  Or->>FS: report.md + all_timings.md
```
...
- [ ] **Add `sweep` CLI** Implement `python -m accelsim_test.gemm_transpose_bench sweep --out-dir tmp/<run_id>` that runs timing only (experiment stage).
- [ ] **Add `report` CLI** Implement `python -m accelsim_test.gemm_transpose_bench report --out-dir tmp/<run_id>` that generates `report.md` + `all_timings.md` from `results.json` (no benchmark run).
...
- Sweep: `<command>`
- Report generation: `pixi run -e cuda13 gemm-transpose report --out-dir <abs_out_dir>`
