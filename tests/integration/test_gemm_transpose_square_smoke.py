...
def test_square_smoke(tmp_path: Path) -> None:
    ...
    square_records = [r for r in results.get("records", []) if r.get("suite") == "square" and r.get("shape", {}).get("m") == 512]
    assert len(square_records) == 5
