...
- One notable boundary still exists: for **square `N=1000` int8**, `ABT_view` selects a different algo (`algo_id=23`) and is ~2.4x faster than `AB` (but note: `A@B.T` is a different math result than `A@B` unless `B` is symmetric).
  - Follow-up usable-algo sweep (AB vs ABT_view, row-major, int8; `N in {1000,1024,2048}`) shows `algo_id=23` is **only usable for ABT_view** under this layout policy, and is **only top-performing at N=1000** (it is usable but not competitive at `N=1024/2048`).
...
### algo 23 across N=1000/1024/2048 (usable-algo sweep)

The full NVBench sweep above records only the **heuristic-selected** algo per case. To understand *which* algo IDs are even eligible (and how they perform) when switching `AB` ↔ `ABT_view`, we ran a focused usable-algo sweep that:

- Enumerates `algo_id` via `cublasLtMatmulAlgoGetIds`
- Derives one selected config per `algo_id` per case via limited heuristic search (`CUBLASLT_SEARCH_LIMITED_BY_ALGO_ID`)
- Classifies usability via `cublasLtMatmulAlgoCheck` under a fixed workspace policy
- Times all usable candidates under consistent warmup/iters

Artifacts (generated by `scripts/cublaslt_usable_algo_sweep.py`): `reports/transpose_matmul/cublaslt_usable_algo_sweep_20260210_131220/`

Key result (time_us; best-of-case among usable candidates):

| N | best AB (algo_id, time_us) | best ABT_view (algo_id, time_us) | `algo_id=23` usable? (AB / ABT_view) | `algo_id=23` rank (ABT_view) |
|---:|---:|---:|---:|---:|
| 1000 | 64, 32.84 | 23, 14.40 | NA / yes | 1/2 |
| 1024 | 70, 8.59 | 70, 7.54 | NA / yes | 4/6 |
| 2048 | 71, 12.37 | 71, 12.40 | NA / yes | 4/6 |

Interpretation:

- `algo_id=23` remains **ABT_view-only** (not usable for `AB`) under the row-major layout policy for these sizes.
- The big win is a **small-shape boundary effect**: `algo_id=23` is *fastest* at `N=1000`, but at `N=1024/2048` it is usable yet **dominated** by the `algo_id=70/71` family.
